...
The Patterson and Hunter (1983) study assumed single stratum block designs and in their example efficiency analysis of 3 replicates of 48 varieties they assumed separate designs for each of the three block sizes 8, 6 or 4 . However, with a hierarchically nested multi-stratum block design it is feasible to assume blocks of size 8 and blocks of size 4 in the same design. The following table shows 20 simulations of the estimated relative efficiency of treatment comparisons for these two block sizes assuming an optimized design for 3 replicates of 48 treatments in complete replicate blocks with six blocks of size 8 in each main block and two blocks of size 4 in each block of size 8. The data was generated by the variance function given by Patterson and Hunter in equation (1) and the analysis was done by using the program shown in Appendix 1 based on the the 'lmerTest' package by Kuznetsa et al (2014) and the lme4 package by Bates et. al. (2014).
...
The results of the simulations show that the optimum block size was sometimes 8 and sometimes 4 with about equal frequency meaning that it is not possible to predict the optimum block size, even when the plot data is based on the same assumed data distribution. In real situations, the error structure will be unknown and the choice of an optimum block size will be even more problematic.  

###  Constrained efficiency

In an hierarchic nested block design, the smaller blocks are constrained by the treatment allocation of the larger blocks therefore there could be a potential loss of efficiency for the small blocks compared with blocks of the same size from a 'conventional' block designs based on a single set of resolvable nested blocks. Our experience is that for medium to large designs any loss of efficiency due to hierarchical nesting is usually negligible. In the Patterson and Hunter example, the upper efficiency bound for blocks of size 4 is .69012 and the best achieved efficiency for blocks of size 4 in a 'classical' resolvable incomplete block design using the 'blocksdesign' package, was .68804 whereas the best achieved efficiency for hierarchically nested blocks of size 4 was .6867. Thus the loss of efficiency on the blocks of size 4 in the hierarchic nested design compared with blocks of size 4 in the 'conventional' design was less than 0.2%. 

As a further example, a design for two replicates of 256 treatments was optimized assuming a maximal hierarchical nesting scheme based on recursive 2-level nesting down to a minimum block size was 2. The following table shows the achieved efficiencies of each blocks stratum when fitted hierarchically and when fitted as a single nested stratum in a 'classical' nested block design with resolvable main blocks. The final column shows upper bounds for each block size in a completely unconstrained block design. For this example, there was virtually no loss of efficiency in any stratum due to the constraints of the hierarchical model.
...
It was shown in the simulation study of the Patterson and Hunter (1983) example that the best block size for an efficient nested block analysis was not predictable and that a multi-stratum design with hierarchical nesting could be used to help select the best single block size for any particular data set. However, it is normal when fitting data with multiple sources of random variation to select a model that includes all the significant random error terms based on an appropriate measure of model fit (see West et al 2014). For large designs with hierarchical multi-stratum nesting, mixed model analysis including more than a single stratum in the analysis may be an appropriate method of analysis. Model building strategies for the inclusion or exclusion of random effects in a model have been extensively researched and the software package 'lmerTest' has an option for backward elimination of non-significant random effects from a model. 

Further discussion of model fitting for large multi-stratum nested block designs is beyond the scope of this vignette and further research is needed for a full understanding of hierarchical nesting in large experiments. However, the 'blocksdesign' package can be used as a tool for exploring some of the implications empirically. For example, as shown above, it is easy to compare the efficiency of optimized nested blocks in a hierarchic nested block design with the efficiency of comparable optimized blocks in a design with a single level of nesting. This approach could be extended to provide power studies to compare designs with different multi-stratum nesting schemes. For example, in the Patterson and Hunter design for 3 replicates of 48 treatments, a design with four nested strata and blocks of size 48, 16, 8, 4 and 2 could be simulated and tested. It is important to ensure that a sufficient number of blocks is available for estimation of the stratum variances at each level of nesting and this scheme provides 9, 18, 36 and 72 blocks for the four nested levels, which seems adequate.        

In conclusion, it can be assumed that provided the constraints of multi-stratum nesting do not cause any meaningful loss of blocks efficiency, a hierarchical multi-stratum design together with an appropriate mixed model analysis of the data has the potential for significant improvement in precision of estimation of large block designs without loss of the traditional robustness of randomized block designs. 
<br/> <br/> 
