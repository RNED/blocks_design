...
Block designs group experimental units into homogeneous blocks which ensure maximum precision for treatment comparisons. The most basic type of block design is the complete randomised blocks design where each experimental block has one or more complete sets of treatments. Complete blocks designs estimate all treatment effects with maximum efficiency and provide excellent designs for small or medium size experiments. However, for large designs, complete blocks are usually too large to provide homogeneous comparisons within the same block and then methods for further reduction of within-block variability may be required.

One well proven method of reducing the variability of large complete blocks is to subdivide each main block into smaller incomplete blocks that are fully nested within individual main block. In an early paper on block designs, Yates (1936) introduced a class of incomplete block designs called balanced lattice block designs. These designs subdivided each complete block of a design for v x v equally replicated treatments into v incomplete blocks of size v. Balanced lattice designs have a high degree of balance and are highly efficient for estimating treatments effects but they exist only for certain specific treatment numbers, specific replication numbers and specific block sizes and are of limited availability. 
