...
### Block size simualations

The Patterson and Hunter (1983) study assumed single stratum block designs therefore in their example designs for 48 varieties they assumed separate designs for each of the three block sizes 8 or 6 or 4 . However, with a hierarchically nested multi-stratum block design it is feasible to assume blocks of size 8 and blocks of size 4 in the same design by nesting blocks of size 8 in each main block of size 48 and nesting blocks of size 4 in each nested block of size 8. The following table shows 20 simulations of the estimated relative efficiency of treatment comparisons for these two block sizes assuming an optimized design with three main replicate blocks, six blocks of size 8 within each main block of size 48 and two blocks of size 4 within each block of size 8. The data was generated by the variance function given by Patterson and Hunter in equation (1) and the analysis was done by using the 'lmerTest' package by Kuznetsa et al (2014) and the lme4 package by Bates et. al. (2014) using the program is shown in Appendix 1. 
