...
# Hierarchic nested block designs
R. N. Edmondson  
18/09/2014  
...
Incomplete block designs improve the within-blocks precision but they also confound treatment information between blocks therefore an efficient analysis must take full account of all the different sources of variability in a design when estimating treatment effects. Before the development of modern computers and modern software, the complexity of the analysis was a major restriction on the choice of incomplete block design but nowadays the availability of modern analysis methods such as the mixed model estimation methods available in the R packages 'lme4' and 'lmerTest' have largely removed this restriction. Modern computer software is capable of routine analysis of very general block designs so there is now no computational limitation on the use of very general block designs that can cope with the multiple sources of variability often found in large experimental designs.


### Design algorithms

Much work has been done on the development of computer algorithms for efficient block design. Cook, R. D. and Nachtsheim, C. J. (1980) gave a review of some of the earlier work up to about 1980. One important development has been the use of swapping algorithms for the construction of efficient block designs for arbitrary number of treatments, arbitrary replication and arbitrary block sizes. Swapping algorithms work by swapping treatments at random between different nested blocks within the same main blocks and retaining improving swaps until no further improvement is possible. A good general purpose swapping algorithm will fit a design for any experimental requirements and will give a good design close to optimal for any required design and any chosen optimality criteria. There are a number of possible optimality criteria available for block designs but the most common are probabaly A-optimality which maximizes the harmonic mean and D-optimality which maximises the geometric mean of the eigenvalues of the design information matrix. Usually the two criteria seem to give similar although not always identical designs. A-optimality seems to have a more direct interpretation than D-optimality (John and Williams 1995 Chapter 2) but D-optimality seems to be more tractable for manipulation and optimization. 


### Block sizes

Patterson and Hunter (1983) reviewed 244 cereal variety trials arranged in generalized incomplete block designs and found that the mean improvement in yield efficiency relative to complete randomized blocks was 1.43. Results varied greatly from one trial to another, however, and the distribution of efficiencies was positively skewed across the set of trials. Precision was more than doubled in one tenth of the trials and in another tenth the lattice arrangement had little effect on accuracy. Nevertheless, in half the trials the efficiency was 1.23 or more. They also modelled the background field variability of 166 of the trials assuming an empirical exponential variance function. 
...
### Hierarchic nested designs

Classical field experiments have only a single level of nesting with sub-blocks nested within main replicate blocks but there is no reason why blocks should not be nested hierarchically with two or more levels of nesting. One advantage of hierarchical nesting is that it provides a range of block sizes within the same design so, if required, a choice of efficient block size can be made according to the actual observed data. The following table shows 20 simulations using data generated by the variance function given by Patterson and Hunter in equation (1). The results show the estimated relative efficiency of treatment comparisons for the two block sizes assuming an optimized hierarchical design with three main replicate blocks and six blocks of size 8 within each main block and two blocks of size 4 within each block of size 8. The analysis was done by using the 'lmerTest' package by Kuznetsa et al (2014) and the lme4 package by Bates et. al. (2014) and the program is shown below in Appendix 1. The results of the simulations show that the optimum block size was sometimes 8 and sometimes 4 with about equal frequency. Thus, even when based on the same assumed data distribution, prediction of the optimum block size is unreliable and cannot be made with confidence before the experimental data has been analysed. In real situations, the error structure will be unknown and the achoice of an optimum block size will be even more problematic.
...
###  Hierarchic efficiency

In an hierarchic nested block design, the smaller blocks are constrained by the larger blocks so there could be a potential loss of efficiency for the small blocks compared with 'conventional' block designs with a single set of resolvable nested blocks. However, our experience is that for medium to large designs any loss of efficiency due to hierarchical nesting is usually negligible. In the Patterson and Hunter trial, for example, the upper efficiency bound for blocks of size 4 is .69012 and the best achieved efficiency for blocks of size 4 in a 'classical' resolvable incomplete block design using the 'blocksdesign' package, was .68804 whereas the best achieved efficiency for hierarchically nested blocks of size 4 was .6867. Thus the loss of efficiency on the blocks of size 4 in the hierarchic design compared with blocks of size 4 in the 'conventional' design was less than 0.2%. 

As a further example a design for two replicates of 256 treatments was optimized assuming a maximal hierarchical nesting into 2-level sub-blocks. The following table shows the achieved efficiencies of each blocks stratum when fitted hierarchically and when fitted separately as a single nested blocks stratum in a 'classical' nested block design with resolvable main blocks. The final column shows upper bounds for each block size as an unconstrained block design. For this eaxample there is virtually no loss of efficiency in any stratum due to the constraints of the hierarchical model.
...
Although a conventional nested block analysis can be used to fit a hierarchically nested design using the most efficient available block size, it is normal when fitting data with multiple sources of random variation to fit models that include all the significant random error terms based on an appropriate measure of model fit (see West et al 2014). Hence, for large designs with two or more levels of hierarchical nesting a mixed model analysis including more than a single stratum in the analysis may be appropriate. Model building strategies for the inclusion or exclusion of random effects in a model have been extensively researched and the software package 'lmerTest' has an option for backward elimination of non-significant random effects from a model. 

Further research is needed for a full understanding of hierarchical nesting in large experiments but the 'blocksdesign' package can be used as a tool for exploring some of the implications empirically. For example, as shown above, it is easy to compare the efficiency of optimized nested blocks in a hierarchic design with the efficiency of comparable optimized blocks in a design with a single level of nesting.  

It seems likely therefore that multi-stratum mixed model analysis of hierarchically nested block designs has the potential for significant improvement in precision of estimation while still retaining the traditional robustness of randomized block designs.    
