...
One method of reducing the variability of large complete blocks is to subdivide each complete block into smaller incomplete blocks that are fully nested within the main blocks.  Yates (1936), in an early paper on block designs, introduced a class of incomplete block designs called balanced lattice designs. These exist if the treatment number v x v is the square of a prime  or prime power v and the nested block size is v and the number of replications is not more than v+1. For 3 or fewer replicates, lattice designs exist for any square number of treatments v x v where the block size is v. Although balanced lattice designs have a high degree of balance and are highly efficient, their limited availability and applicability make them of limited practical value.
...
It was shown in the simulations for the Patterson and Hunter (1983) example that the best block size for an efficient nested block analysis was not predictable. By using a multi-stratum design with an appropriate analysis it is possible to select the single best block size from a range of block sizes. However, it is normal when fitting data with multiple sources of random variation to select models that include all significant random error terms in the model therefore for large designs, a mixed model analysis including all the significant random error terms might be an appropriate method of analysis. Model building strategies for the inclusion or exclusion of random effects in a model have been extensively researched and the software package 'lmerTest' has an option for backward elimination of non-significant random effects from a model. 

Further discussion of model fitting is beyond the scope of this vignette and further research is needed for a better understanding of multi-stratum nested designs in large experiments. However, the 'blocksdesign' package can be used as an empirical tool for exploring some of the design implications. For example, as shown above, it is easy to compare the efficiency of optimized nested blocks in a multi-stratum nested block design with the efficiency of comparable optimized blocks in a design with a single level of nesting. This approach could be extended to provide power studies to compare designs with different multi-stratum nesting schemes. In the Patterson and Hunter example, there were for 3 replicates of 48 treatments so a design with four nested strata and blocks of size 48, 16, 8, 4 and 2 could be simulated and tested. It is important to ensure that a sufficient number of blocks is available for estimation of the stratum variances at each level of nesting and in this scheme there are 9, 18, 36 and 72 blocks for the four nested levels, which seems adequate.        

In conclusion, provided the constraints of multi-stratum nesting do not cause substantive loss of blocks efficiency, a hierarchical multi-stratum design together with an appropriate mixed model analysis of the data has the potential for significant improvement in precision of estimation of large block designs without loss of the traditional robustness of the randomized block design. 
<br/> <br/> 
